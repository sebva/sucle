		...
		try
		{
			...
			if (jObject.getString(WebServicesInfo.JSONKey.STATUS).equals(WebServicesInfo.JSONKey.STATUS_VALID))
			{
				...
				for (int i = 0; i < jArray.length(); ++i)
				{
					...
					User user = new User(userObject.getString(WebServicesInfo.JSONKey.USER_SOCIAL_ID), userObject.getString(WebServicesInfo.JSONKey.USER_NAME), socialType, new SimpleDateFormat("yyyy-MM-dd HH:mm:ss", Locale.ENGLISH).parse(userObject
					...
					posts.add(new Post(Integer.valueOf(id), parent, user, new LatLng(Double.valueOf(object.getString(WebServicesInfo.JSONKey.MESSAGE_LAT)), Double.valueOf(object
